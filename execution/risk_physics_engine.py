...
- 이 파일에서 말하는 effective_risk_pct 는 이제
...
- 정책 기반의 "명시적" 보정만 허용:
  - DD/연속손실/최소 RR 등 사전에 정의된 규칙에 의한 감쇠/차단.
- 민감정보 로그 금지.
- settings 객체 불변(읽기만).
...
1) Regime allocation(0.0~1.0) 은 caller가 공급한다.
   - 예: NO_TRADE=0.0, LOW=0.4, MID=0.7, HIGH=1.0
2) DD 감쇠(Allocation multiplier)
   - DD >= 5%  -> x0.8
   - DD >= 10% -> x0.5
   - DD >= 15% -> 신규 진입 차단(SKIP)
   - DD >= 20% -> 엔진 정지 권고(STOP)  (caller가 SAFE_STOP 처리)
...
class RiskPhysicsEngine:
    ...
    - RiskPhysicsDecision(action_override, effective_risk_pct, ...)
      * action_override == "ENTER": 실행 레이어가 ENTER 진행
      * action_override == "SKIP" : 신규 진입 차단
      * action_override == "STOP" : 엔진 정지 권고(즉시 SAFE_STOP 처리 권장)
    """
        ...
        *,
        ...
        if alloc_adj <= 0.0:
            return RiskPhysicsDecision(
                ...
                planned_rr=rr,
                reason="effective_allocation_zero_after_adjustments",
            )
        ...
        # effective_risk_pct is allocation itself
        return RiskPhysicsDecision(
