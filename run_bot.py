...
지금까지 쪼갠 모듈들을 모두 합쳐 실제로 봇을 실행하는 진입점이다.
원래 하나의 bot.py 안에 있던 흐름을 최대한 유지하면서, 모듈로 나뉜 부분만 import 로 교체했다.

구성:
- 설정 로드 및 전역 상태 초기화
- health/metrics HTTP 서버 (옵션)
- 거래소 포지션/주문과 내부 상태 동기화
- 메인 while 루프:
  - 잔고 주기적 로그
  - KST 자정 일일 정산
  - 열린 포지션 TP/SL 체결 확인
  - 연속 손실/쿨다운/시간대 필터
  - 3m → (추세) → 15m 방향 일치 → (옵션) 1m 확인
  - 추세 신호 없으면 박스장 신호
  - 진입 전 각종 가드: 캔들 지연, 동일 캔들 중복, 급등락, 스프레드, 잔고 0, ATR 급등 시 리스크 축소
  - 시장가 진입 + TP/SL 예약

주의:
- 여기서는 실제 주문을 보내는 부분을 trader.py / exchange_api.py 로 다 넘겼다.
- 이 파일만 실행하면 전체 봇이 돌아간다.
"""
...
LAST_CLOSE_TS: float = 0.0  # 전체 최근 청산 시각
CONSEC_LOSSES: int = 0      # 연속 손실 카운트
CONSEC_KLINE_FAILS: int = 0 # 시세 조회 실패 연속 카운트

# 박스장 하루 손절 카운트 & 비활성화 플래그
...
# ─────────────────────────────class _HealthHandler(BaseHTTPRequestHandler):
    """/healthz, /metrics 를 간단히 제공하는 핸들러"""

    def do_GET(self):  # noqa: N802 (BaseHTTPRequestHandler 규칙)
...
def start_health_server() -> None:
    """환경설정에 포트가 지정돼 있으면 health 서버를 띄운다."""
...
SESSIONS = _parse_sessions(SET.trading_sessions_utc)


def in_trading_session_utc() -> bool:
    """현재 UTC 시간이 설정해 둔 트레이딩 세션 안에 있는지 확인한다."""
...
# ─────────────────────────────def _sigterm(*_: Any) -> None:
    """SIGTERM 이 들어오면 자연스럽게 종료하도록 플래그만 바꾼다."""
...
# ─────────────────────────────def sync_open_trades_from_exchange(symbol: str) -> None:
    """거래소에 이미 열려 있는 포지션 + 조건부 주문을 가져와서 OPEN_TRADES 에 맞춰 넣는다.

    - 포지션이 없으면 아무것도 안 한다.
    - 기존 bot.py 와 동일하게 source="SYNC" 로 표시해서 박스장 일일 손절 카운트에는 안 들어가게 한다.
    """
    global OPEN_TRADES
    ...
    for pos in positions:
        ...
        if qty == 0.0:
            continue
        side = (pos.get("positionSide") or pos.get("side") or "").upper()
        side_open = "BUY" if side in ("LONG", "BUY") else "SELL"
        ...
        sl_price = None
        for o in orders:
            o_type = o.get("type") or o.get("orderType") or ""
...
# 메인
# ─────────────────────────────def main() -> None:
    global RUNNING, OPEN_TRADES, LAST_CLOSE_TS, CONSEC_LOSSES, CONSEC_KLINE_FAILS
    ...
    if not SET.api_key or not SET.api_secret:
        msg = "❗ BINGX_API_KEY 또는 BINGX_API_SECRET 이 비어있습니다. 환경변수를 다시 설정하세요."
        log(msg)
        send_tg(msg)
        return

    log(
        f"CONFIG: ENABLE_TREND={SET.enable_trend}, ENABLE_RANGE={SET.enable_range}, "
    ...
    while RUNNING:
        try:
            ...
            # KST 자정에 하루 정산 메시지 보내기 (00:00~00:00:59 사이 1회)
            now_kst = datetime.datetime.now(KST)
            ...
            if now - last_fill_check >= SET.poll_fills_sec:
                ...
                for c in closed_list:
                    t: Trade = c["trade"]
                    reason: str = c["reason"]
                    summary: Optional[Dict[str, Any]] = c.get("summary")
                ...
                # TP/SL 재설정이 연속으로 실패해서 봇을 멈춰야 하는 경우
                if TRADER_STATE.should_stop_bot():
                    send_tg(
                        "🛑 TP/SL 재설정이 연속으로 실패해서 봇을 중단합니다. 거래소 상태를 확인하세요."
                    )
                    RUNNING = False
                    break

            # 포지션이 열려 있으면 새로 진입하지 않음
            if OPEN_TRADES:
                time.sleep(1)
                continue
            ...
            if now_ms - latest_3m_ts > SET.max_kline_delay_sec * 1000:
                send_skip_tg(
                    "[SKIP] 3m_kline_delayed: 최근 3분봉 캔들이 지연되어 진입하지 않습니다."
                )
                time.sleep(1)
                continue
                    ...
                    if (time.time() - LAST_CLOSE_TS_TREND) >= SET.cooldown_after_close_trend:
                        ...
                        if SET.enable_1m_confirm:
                            ...
                            if not confirm_1m_direction(candles_1m, chosen_signal):
                                send_skip_tg(
                                    "[SKIP] 1m_confirm_mismatch: 1분봉이 진입 방향과 맞지 않습니다."
                                )
                                chosen_signal = None
                                signal_source = None
                    else:
                        send_skip_tg(
                            "[SKIP] trend_cooldown: 직전에 TREND 포지션을 청산해서 대기 중입니다."
                        )

            # 2) 박스장 전략 (추세 신호 없을 때만)
            if (not chosen_signal) and SET.enable_range and (not RANGE_DISABLED_TODAY):
                ...
                if (time.time() - LAST_CLOSE_TS_RANGE) >= SET.cooldown_after_close_range:
                    if not should_block_range_today(candles_3m, candles_15m):
                        ...
                        send_skip_tg(
                            "[SKIP] range_blocked_today: ATR 수축/15m 이격으로 박스장 진입 스킵"
                        )
                else:
                    send_skip_tg(
                        "[SKIP] range_cooldown: 직전에 RANGE 포지션을 청산해서 대기 중입니다."
                    )
            ...
            if move_pct > SET.max_price_jump_pct:
                send_skip_tg(
                    f"[SKIP] price_jump_guard: 캔들 간 급등락 {move_pct:.4f} > {SET.max_price_jump_pct:.4f}"
                )
                time.sleep(1)
                continue
            ...
            if SET.use_atr:
                ...
                if atr_fast and atr_slow and atr_slow > 0:
                    if atr_fast > atr_slow * SET.atr_risk_high_mult:
                        ...
                        log(
                            f"[ATR RISK] fast ATR high → risk reduced {SET.risk_pct} -> {effective_risk_pct}"
                        )
            ...
            if signal_source == "RANGE":
                ...
                if SET.use_atr:
                    atr = calc_atr(candles_3m, SET.atr_len)
                    if atr and last_price > 0:
                        sl_pct_atr = (atr * SET.atr_sl_mult) / last_price
                        tp_pct_atr = (atr * SET.atr_tp_mult) / last_price
            ...
            send_tg(
                ...
                f"레버리지={SET.leverage}x 사용비율={effective_risk_pct*100:.0f}% 명목가≈{notional:.2f}USDT 수량={qty}"
            )
