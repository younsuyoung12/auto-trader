...
3) EV Heatmap Block
- cell_status == BLOCK → block_entry=True (단, micro_score_risk가 매우 낮아도 EV block은 우선)
...
--------------------------------------------------------
- 2026-03-03:
  1) 신규 생성: 마이크로구조 + EV 히트맵 기반 자동 차단/감쇠 엔진(STRICT)
========================================================
"""
...
def decide_auto_block(
    *,
    ...
    if st == "NOT_READY":
        ...
        ev_val = _require_float(stage, heatmap_ev, "heatmap_ev")
    ...
    if st == "BLOCK":
        ...
        # Still compute multiplier for logging, but block overrides
    ...
    if consecutive_losses is not None and consec_loss_penalty_threshold is not None:
        ...
        if cl >= th and not block_entry:
            # deterministic extra penalty (0.7)
            risk_mul *= 0.7
